    ...
    for item in data:
        ...
        try:
            ...
            workorder, created = WorkOrder.objects.get_or_create(
                ...
                created_on= timezone.make_aware(datetime.strptime(item['Created on'], "%Y-%m-%d %H:%M:%S")) if item['Created on'] else None,
                first_due_date= timezone.make_aware(datetime.strptime(item['Due date'], "%Y-%m-%d %H:%M:%S")) if item['Due date'] else timezone.now(),
            ...
            if created:
                print(f"WorkOrder {item['Title']} recurence {item['Recurrence']} created successfully")
            else:
                # update attachment
                if attachmentUrl:
