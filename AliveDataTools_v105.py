...
#
# 2024.10.09 Version 105
#   changes for Acumatica Version 24R1
#   add header variable to login request
...
#BASE_URL = "https://westridgelabs-sandbox-24-1.acumatica.com"
BASE_URL = "https://westridgelabs.acumatica.com"
# BASE_URL  = "http://10.1.1.22/alive"

#ENDPOINT_URL  = "/entity/Default/20.200.001/"
ENDPOINT_URL  = "/entity/Default/22.200.001/"
LOGIN_URL     = "/entity/auth/login"
LOGOUT_URL    = "/entity/auth/logout"
...
class RestQuery:
    ...
    def __init__(self, debug=False):
        ...
        self.debug = debug
        self.endpoint_url = BASE_URL + ENDPOINT_URL
        ...
        with requests.Session() as self.s:
            response = self.s.post(BASE_URL + LOGIN_URL, data=payload, headers=headers, timeout=300)
            if self.debug: print ('login', response)

    def __del__(self):
        with requests.Session() as s:
            response = s.post(BASE_URL + LOGOUT_URL, timeout=10)
    ...
    # ******************************************************************************************************************
        ...
        if type == 'grid':
            ...
            response = self.s.get(rest_query, timeout=300)
            # print("*** QUERY ***")
            # print(response)
            ...
            response = self.s.get(id_query, timeout=300)
            #print("*** QUERY ***")
            #print(response)   
...
if __name__ == "__main__":
    ...
    #
    if True:
        print('\n*** AliveDataTools Test #1, ODATA ***')
        odata_filter = "InventoryCD eq 'IDDGLD08AA'"
        odata_select = ''\
            'InventoryCD,'\
            'Description,'\
            'UnitRate,'\
    ...
    # 2 REST API Queries Inventory stock items with Attributes
    #
    if True:
        print('\n*** AliveDataTools Test #2, Stock Items ***')
        rs = RestQuery(debug=True)
        result = rs.query(
            table  ='StockItem',
            filter = '',
            expand ='Attributes, UOMConversions',
            fields ='InventoryID,Description,Attributes/AttributeID,Attributes/ValueDescription'
        )
        
        for row in result:
            inventoryID = row['InventoryID']['value']
            descr_v = row.get('Description', None)
            description = descr_v['value'] if descr_v else 'NO DESCRIPTION'
            back_order_til = None
            ...
            if back_order_til:
                print(f'{inventoryID}: {description} || back in stock:{back_order_til}')

        # for line in result:
        #     print(line)
        del rs

    # 3 REST API Queries with single REST Query Object
    #
    if True:
        print('\n*** AliveDataTools Test #3, REST API ***')
        #skus = ['IDDGLD08','IDDGLD04','ASRT1024']
        skus = ['IDDGLD08C2','IDDGLD08AA',]
        rs = RestQuery(debug=True)
        sku_data_list = []
        progress_counter = 1
        ...
        for sku in skus:
            ...
            result = rs.query(
                ...
                expand ='Attributes, UOMConversions',
                fields ='InventoryID,TemplateItemID,Description,SalesUOM,UOMConversions/ConversionFactor,UOMConversions/FromUOM,Attributes/AttributeID,Attributes/ValueDescription'
            )
            if verbose:
                print('result:')
                print(result)
    ...
    # 4 REST API Get Inventory and Attributes
    #
    if True:
        print('\n*** AliveDataTools Test #4, get_inventory() ***')
        rs = RestQuery(debug=False)
        result = rs.query(
            table  ='StockItem',
            filter = '',
            expand ='Attributes, UOMConversions',
        ...
        # generate file line for items on backorder today: date, sku1, sku2, sku3 ...
        #
        backorder_sku_list = []
        for row in result:
            InventoryID = row['InventoryID']['value']
    ...
    #
    if False:
        print('\n*** AliveDataTools Test #5, get, single item ***')
    ...
    # 6 simple REST API Query
    #
    if True:
        print('\n*** AliveDataTools Test #6, simple REST API ***')
        rs = RestQuery(debug=True)
        result = rs.query(
            table  ='StockItem',
            filter = '',
        ...
        for row in result:
            ...
            descr_v = row.get('Description', None)
            description = descr_v['value'] if descr_v else 'NO DESCRIPTION'
            if inventoryID[:4] == 'IDDG':
                print(inventoryID, description)
